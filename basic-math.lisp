...
(defun make-permuatation-swap-iterator (n)
  (let ((b (coerce (iter (for i from 0 to (1- n)) (collect i)) 'vector))
	(c (coerce (iter (for i from 0 to n) (collect 0)) 'vector)))
    (lambda ()
      (let ((k 1))
	(iter (while (= (aref c k) k))
	      (setf (aref c k) 0)
	      (incf k))
	(unless (= k n)
	  (incf (aref c k))
	  (let ((result (aref b k)))
	    ...
	    (let ((j 1))
	     (iter (while (< j k))
		   (let ((temp (aref b j)))
		     (setf (aref b j) (aref b k))
		     (setf (aref b k) temp)
		     (decf k)
		     (incf j))))
	    result))))))

(defmacro with-permutations-swap ((var vec) &body body)
  (let ((continue (gensym "cont"))
	(permutator (gensym "perm"))
	(temp (gensym "t")))
    `(let ((,permutator (make-permuatation-swap-iterator (length ,vec)))
	   (,continue t)
	   (,var (copy-seq ,vec))
	   ,temp)
       ,@body
       (iter (while ,continue)
	     (setf ,continue (funcall ,permutator))
	     (when ,continue
	       (setf ,temp (aref ,var ,continue))
	       (setf (aref ,var ,continue) (aref ,var 0))
	       (setf (aref ,var 0) ,temp)
	       ,@body)))))
